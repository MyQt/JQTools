...
float FinderPatternFinder::crossCheckHorizontal(size_t startJ, size_t centerI, int maxCount,
  ...
  int j = (int)startJ;
  while (j >= 0 && image_->get((int)j, (int)centerI)) {
    stateCount[2]++;
    j--;
  }
  if (j < 0) {
  ...
  while (j >= 0 && !image_->get((int)j, (int)centerI) && stateCount[1] <= maxCount) {
    stateCount[1]++;
    j--;
  }
  ...
  while (j >= 0 && image_->get((int)j, (int)centerI) && stateCount[0] <= maxCount) {
    stateCount[0]++;
    j--;
  }
  if (stateCount[0] > maxCount) {
  ...
  j = (int)startJ + 1;
  while (j < maxJ && image_->get((int)j, (int)centerI)) {
    stateCount[2]++;
    j++;
  }
  if (j == maxJ) {
  ...
  while (j < maxJ && !image_->get((int)j, (int)centerI) && stateCount[3] < maxCount) {
    stateCount[3]++;
    j++;
  }
  ...
  while (j < maxJ && image_->get((int)j, (int)centerI) && stateCount[4] < maxCount) {
    stateCount[4]++;
    j++;
  }
  if (stateCount[4] >= maxCount) {
